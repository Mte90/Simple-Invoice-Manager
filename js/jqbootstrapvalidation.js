...
(function( $ ){
	...
	var defaults = {
		options: {
			...
			submitError: false,
			submitSuccess: false,
 //Part modified for support the modal window
			event:null,
 //Part modified for support the modal window
			autoAdd: {
				helpBlocks: true
	...
	var $form = $('#'+$(this).parent().data('id-form'));
	var warningsFound = 0;
	...
	if (warningsFound) {
		if (settings.options.preventSubmit) {
			settings.options.event.stopPropagation();
			settings.options.event.stopImmediatePropagation();
			settings.options.event.preventDefault();
		}
		$form.addClass("error");
